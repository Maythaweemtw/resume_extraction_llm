...
# Function descriptions and prompt template remain the same as before
...
def append_to_csv(data, filename="resumes.csv"):
    ...
    with open(filename, mode='a', newline='', encoding='utf-8') as file:
        ...
        if not file_exists:
            header = ["Name", "Email", "Phone", "Education", "Employment", "Skills"]
            writer.writerow(header)
        
        education = "; ".join([f"{edu['school']} - {edu['degree_or_certificate']} ({edu['time_period']})" for edu in data['education']])
        employment = "; ".join([f"{emp['company']} - {emp['title']} ({emp['time_period']})" for emp in data['employment']])
        skills = ", ".join(data['skills'])
        
        row = [data['name'], data['email'], data['phone'], education, employment, skills]
        writer.writerow(row)
